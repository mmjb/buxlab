# Taken from fs-mol; but uses patlib.Path rather than dpu_utils.RichPath.
...
QueuePathType = TypeVar("QueuePathType")
QueueReaderOutputType = TypeVar("QueueReaderOutputType")


def read_file_from_path_queue(
    input_paths: QueuePathType,
    output_queue: QueueReaderOutputType,
    reader_fn: Callable[[List[Path], int], Iterable[ReaderOutputType]],
...
class BufferedFileReaderIterator(Iterator[ReaderOutputType]):
    ...
    def __init__(
        ...
        self._file_queue: "Queue[Path]" = Queue(len(self._data_paths))
